...
/** Phase where we do cps transformation. Note, that this phase should run before inlining, because when we search for async-shift
  * object, it can be inlined.
  * @param settings
  * @param selectedNodes
  */
class PhaseCps(settings: CpsPluginSettings, selectedNodes: SelectedNodes) extends PluginPhase {
  ...
  override def transformDefDef(tree: tpd.DefDef)(using Context): tpd.Tree = {
    selectedNodes.getDefDefRecord(tree.symbol) match
      case Some(selectRecord) if (!selectRecord.internal) =>
        try transformDefDefInternal(tree, selectRecord, None)
        catch
  ...
  def transformDefDefInternal(
      tree: DefDef,
      selectRecord: DefDefSelectRecord,
      optTopLevelContext: Option[CpsTopLevelContext] = None
    ...
    val retval = selectRecord.kind match
      case USING_CONTEXT_PARAM(cpsMonadContextArg) =>
        ...
        if (tree.rhs.isEmpty) then
          selectRecord.monadType =
            CpsTransformHelper.extractMonadType(cpsMonadContext.tpe.widen, CpsTransformHelper.cpsDirectAliasSymbol, tree.srcPos)
          tree
        else
          val (tc, monadValDef) = makeCpsTopLevelContext(
            cpsMonadContext,
            tree.symbol,
            tree.srcPos,
            debugSettings,
            CpsTransformHelper.cpsDirectAliasSymbol
          )
          ...
          if (debugSettings.printCode) then Log.info(s"transformDefDefInternal: transformed: ${retval.show}", 0, tree.srcPos)
          retval
  ...
  override def transformApply(tree: tpd.Apply)(using Context): tpd.Tree = {
    try transformApplyInternal(tree)
    catch
      case ex: CpsTransformException =>
        report.error(ex.message, ex.pos)
        throw ex;
    ...
    tree match
      ...
      case Apply(
        ...
        if (ddef.symbol.owner != ctx.owner) {
          report.warning(
            s"transformApplyInternal: ddef.symbol.owner != ctx.owner  ddef.symbol.owner=${ddef.symbol.owner.showFullName}, ctx.owner=${ctx.owner.showFullName}",
            tree.srcPos
          )
        }
        val nClosure = Closure(closure.env, ref(nDefDef.symbol), EmptyTree).withSpan(closure.span)
      ...
      case app @ Apply(tapp @ TypeApply(adoptCpsedCallCompileTimeOnlyCn, List(tf, ta)), List(a))
          if (adoptCpsedCallCompileTimeOnlyCn.symbol == Symbols.requiredMethod(
            "cps.plugin.scaffolding.adoptCpsedCallCompileTimeOnly"
          )) =>
        val adoptCpsedCall =
          ref(Symbols.requiredMethod("cps.plugin.scaffolding.adoptCpsedCall")).withSpan(adoptCpsedCallCompileTimeOnlyCn.span)
          ...
          case ai @ Inlined(call, bindings, expansion) =>
            ...
            findCpsDirectArg(call) match
              case Some(tree) =>
                val (tc, monadValDef) = tree match
                  case CpsDirectHelper.ByInclusionCall(tf, tg, fctx, fginc) =>
                    makeCpsTopLevelContext(
                      fctx,
                      summon[Context].owner,
                      a.srcPos,
                      DebugSettings.make(a, settings),
                      CpsTransformHelper.cpsMonadContextClassSymbol
                    )
              ...
              case None =>
                ...
                report.warning(
                  s"Unexpected argumnet of cps.plugin.scaffolding.adoptCpsedCall (should be apply) : ${a.show}",
                  a.srcPos
                )
      ...
      case Apply(Apply(TypeApply(deferredAsyncCn, List(tp, mtp, mctp)), List(applyTerm)), List(ctx))
        ...
        val (tc, monadValDef) = makeCpsTopLevelContext(
          ...
          summon[Context].owner,
          tree.srcPos,
          DebugSettings.make(tree, settings),
          CpsTransformHelper.cpsMonadContextClassSymbol
        )
        val nApplyTerm = {
          given CpsTopLevelContext = tc
  ...
  private def transformDefDefInsideAsync(ddef: DefDef, asyncCallTree: Tree, contextual: Boolean)(using ctx: Context): DefDef = {
    ...
    val monadType = CpsTransformHelper.extractMonadType(
      cpsMonadContext.tpe.widen,
      CpsTransformHelper.cpsMonadContextClassSymbol,
      asyncCallTree.srcPos
    )
    ...
    if (true) {
      TransformUtil.findFirstSubtermWithIncorrectOwner(ddef.rhs, ddef.symbol) match
        case Some(TransformUtil.IncorrectOwnerRecord(contextOnwer, expectedOwner, realOwner, tree)) =>
          report.debugwarn(
            s"found subterm with incorrect owner: ${tree.symbol}(${tree.symbol.hashCode()}  owner=${realOwner}(${realOwner.hashCode()})",
            ddef.srcPos
          )
        case None =>
    }
    ...
    val contextParam = cpsMonadContext match
      ...
      case _ =>
        throw CpsTransformException(
          s"excepted that cpsMonadContext is ValDef, but we have ${cpsMonadContext.show}",
          asyncCallTree.srcPos
        )
    val (tctx, monadValDef) = makeCpsTopLevelContext(
      contextParam,
      ddef.symbol,
      asyncCallTree.srcPos,
      DebugSettings.make(ddef, settings),
      CpsTransformHelper.cpsMonadContextClassSymbol
    )
    ...
    val nDefDef = DefDef(
      ...
      paramss => {
        ...
        val nBody = TransformUtil
          .substParams(body, ddef.paramss.head.asInstanceOf[List[ValDef]], paramss.head)
  ...
  private def transformDefDef1InsideCpsAsyncStreamApply(funfun: Tree, owner: Symbol)(using ctx: Context): Tree = {
    funfun match
      ...
      case Block((ddef: DefDef) :: Nil, closure: Closure) =>
        ...
        val mt = MethodType(ddef.paramss.head.map(_.name.toTermName))(
          _ => ddef.paramss.head.map(_.typeOpt.widen),
          _ => nInternalLambda.tpe.widen
        )
        val newSym = Symbols.newAnonFun(owner, mt, ddef.span)
        val nDefDef = DefDef(
          newSym,
          { paramss =>
            val retval = TransformUtil
              .substParams(nInternalLambda, ddef.paramss.head.asInstanceOf[List[ValDef]], paramss.head)
              .changeOwner(ddef.symbol, newSym)
            retval
          }
        )
  ...
  private def transformDefDef2InsideCpsAsyncStreamApply(fun: Tree, ctxRef: Tree)(using ctx: Context): Tree = {
    fun match
      ...
      case Block((ddef: DefDef) :: Nil, closure: Closure) if (ddef.symbol == closure.meth.symbol) =>
        ...
        val nRhs =
          Block(monadValDef.changeOwner(monadValDef.symbol.owner, ddef.symbol) :: Nil, nRhsCps.transformed(using ddefContext, tctx))
        val mt =
          MethodType(ddef.paramss.head.map(_.name.toTermName))(_ => ddef.paramss.head.map(_.typeOpt.widen), _ => nRhs.tpe.widen)
        val newSym = Symbols.newAnonFun(ctx.owner, mt, ddef.span)
        val nDefDef = DefDef(
          newSym,
          { paramss =>
            val retval = TransformUtil
              .substParams(nRhs, ddef.paramss.head.asInstanceOf[List[ValDef]], paramss.head)
              .changeOwner(ddef.symbol, newSym)
            retval
          }
        )
  ...
  def wrapTopLevelCpsTree(cpsTree: CpsTree)(using Context, CpsTopLevelContext): Tree = {
    ...
    /** Just for correct diagnosting, aboput applying normalization (which we want to eliminate to normalize 'on fly')
      * @param cpsTree
      * @return
      */
    ...
    cpsTree.unpure match
      ...
      case None =>
        tryNormalize(cpsTree).asyncKind match
          ...
          case _ =>
            throw CpsTransformException(
              s"unsupported type for top-level wrap, asyncKind=${cpsTree.asyncKind}",
              cpsTree.origin.srcPos
            )
  }
  ...
  /** create cps top-level context for transformation.
    ...
    */
  private def makeCpsTopLevelContext(
      cpsDirectOrSimpleContext: Tree,
      owner: Symbol,
      srcPos: SrcPos,
      debugSettings: DebugSettings,
      wrapperSym: Symbol
  )(using Context): (CpsTopLevelContext, ValDef) = {
    ...
    val optRuntimeAwaitProvider =
      if (settings.useLoom) CpsTransformHelper.findRuntimeAwaitProvider(monadType, srcPos.span) else None
    ...
    val tc = CpsTopLevelContext(
      monadType,
      monadRef,
      cpsDirectOrSimpleContext,
      optRuntimeAwait,
