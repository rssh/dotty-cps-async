...
/** Record for defDef, which was selected for transformation. Note, that this shouln not be case class, because it used in
  * MutableSymbolMap and it is important to have equals/hashCode methods not depended from value (i.e. - system).
  * @param kind
  * @param internal
  */
class DefDefSelectRecord(
...
class SelectedNodes {
  ...
  def defDefKindIfTopLevel(sym: Symbol): Option[DefDefSelectKind] = {
    allDefDefs.get(sym).flatMap(r => Option.when(!r.internal)(r.kind))
  }
...
object SelectedNodes {
  ...
  def detectDefDefSelectKind(tree: DefDef)(using Context): Option[DefDefSelectKind] = {
    val optKind = SelectedNodes.checkAndProcessDefDef(tree) { (tree, monadContext) =>
      Some(DefDefSelectKind.USING_CONTEXT_PARAM(monadContext))
    } { (tree, kind) =>
      Some(DefDefSelectKind.RETURN_CONTEXT_FUN(kind))
    }
    optKind
  }
  ...
  /** @param tree:
    *   tree to process
    * @param f:
    *   (defDef(selected function), Tree: (CpsMonadContext parameter), index:Int) => Option[A] is called when we find parameter with
    *   type CpsMonadContext.
    * @param acc
    * @param Context
    * @tparam A
    * @return
    */
  def checkAndProcessDefDef[A](
      tree: DefDef
  )(f: (DefDef, Tree) => Option[A])(acc: (DefDef, A) => Option[A])(using Context): Option[A] = {
  ...
  @tailrec
  private def findAllCpsDirectContextParam(paramss: List[Trees.ParamClause[Types.Type]], acc: List[ValDef])(using
      Context
  ): List[Tree] = {
    paramss match
          ...
          case paramsHead :: paramTail =>
            paramsHead match
              case vd: Trees.ValDef[?] =>
                val filtered =
                  paramssHead.asInstanceOf[List[ValDef]].filter((p: ValDef) => CpsTransformHelper.isCpsDirectType(p.tpt.tpe))
