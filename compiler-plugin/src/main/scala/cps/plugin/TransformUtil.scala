...
object TransformUtil {
  ...
  def realWiden(tpe: Type)(using Context): Type =
    tpe match
      case tpeOr @ OrType(tp1, tp2) =>
        ...
        if (tp1w eq tp1) && (tp2w eq tp2) then tpe
        else OrType.make(tp1w, tp2w, tpeOr.isSoft)
      case _ => tpe.widen
  ...
  def methodTypeFromFunctionType(candidate: Type, srcPos: SrcPos)(using Context): Option[MethodType] = {
    candidate match
      ...
      case _ =>
        if (candidate.baseType(defn.Function0).exists) then Some(MethodType(Nil)(x => Nil, x => candidate))
  ...
  def findSubtermWithOwner(tree: Tree, owner: Symbol, reportWarnings: Boolean = false)(using Context): Option[Tree] = {
    val finder = new TreeAccumulator[Option[Tree]] {
          ...
          tree match
            case xi: Ident if xi.symbol.maybeOwner.exists =>
              if (xi.symbol == owner) then
                if (reportWarnings) then report.warning(s"found ident $xi with direct old owner ${owner} ", tree.srcPos)
                Some(tree)
              else {
                findIndirectOwner(xi.symbol, owner, Nil) match
                  case Some(path) =>
                    if (reportWarnings) then report.warning(s"found ident with indirect owner, path=${path}", tree.srcPos)
                    Some(tree)
                  case None =>
                    foldOver(x, tree)
              }
            case _ =>
              if (tree.symbol.maybeOwner == owner) then Some(tree)
              else
                findIndirectOwner(tree.symbol.maybeOwner, owner, Nil) match
                  case Some(path) =>
                    if reportWarnings then
                      report.warning(
                        s"found tree with indirect old owner, path=${path.reverse.map(s => s"${s}(${s.hashCode()})").mkString("->")}",
                        tree.srcPos
                      )
                    Some(tree)
                  case None =>
      ...
      def findIndirectOwner(x: Symbol, mbOwner: Symbol, path: List[Symbol]): Option[List[Symbol]] = {
        if (x == mbOwner) then Some(x :: path)
        else if (!x.maybeOwner.exists) then None
        else findIndirectOwner(x.maybeOwner, mbOwner, x :: path)
      }
    ...
    val finder = new TreeAccumulator[Option[Tree]] {
      ...
      def apply(x: Option[Tree], tree: Tree)(using Context): Option[Tree] =
        if (x.isDefined) then x
          ...
          tree match
            case xdef: MemberDef =>
              if (!checkOwner(xdef.symbol, owner)) then Some(tree)
              else x
            case _ =>
              foldOver(x, tree)
    }
