...
object AsynchronizedTransform {
  ...
  def fromApply(term: Apply, owner: Symbol, nesting: Int, monadType: Tree, valueType: Tree, internalTerm: Tree, directRef: Tree)(
      using
      Context,
      CpsTopLevelContext
  ): CpsTree = {
  ...
  def parseLambda(
    ...
    val nLambda = Closure(
      ...
      tss => {
        ...
        val nDirect =
          ValDef(directSym, CpsDirectHelper.genCpsDirectDefaultConstructor(monadTypeTree, internalContextParam, term.span))
            .withSpan(term.span)
        Block(
          List(nDirect),
