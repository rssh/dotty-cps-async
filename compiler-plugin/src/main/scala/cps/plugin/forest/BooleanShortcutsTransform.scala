...
object BooleanShortcutsTransform {
  ...
  def apply(term: Apply, owner: Symbol, nesting: Int, obj: Tree, methodSymbol: Symbol)(using Context, CpsTopLevelContext): CpsTree = {
     ...
     val retval = (xCps.asyncKind,yCps.asyncKind) match
       case (AsyncKind.Sync, AsyncKind.Sync) =>
         if (xCps.isOriginEqSync && yCps.isOriginEqSync) then
           CpsTree.unchangedPure(term,owner)
         else
           val nTerm =
                if (xCps.isOriginEqSync) then
                    cpy.Apply(term)(term.fun, List(yCps.unpure.get))
                else if (yCps.isOriginEqSync) then
                    cpy.Apply(term)(Select(obj,methodSymbol.name.toTermName), term.args)
                else
                    cpy.Apply(term)(Select(xCps.unpure.get,methodSymbol.name.toTermName), List(yCps.unpure.get))
       ...
       case (AsyncKind.Async(internalKind), AsyncKind.Sync) =>
         ...
         MapCpsTree(term, owner, xCps,
           MapCpsTreeArgument(Some(valDef), CpsTree.pure(term ,owner,
             generateShortcutIf(term ,refSym, yCps.unpure.get, methodSymbol, false))))
       case (AsyncKind.Async(internalKind1), AsyncKind.Async(internalKind2)) =>
          val sym = newSymbol(owner, "c".toTermName, Flags.EmptyFlags, defn.BooleanType)
          val valDef = ValDef(sym.asTerm, EmptyTree)
          val refSym = ref(sym).withSpan(x.span)
          FlatMapCpsTree(term, owner, xCps,
            FlatMapCpsTreeArgument(Some(valDef), CpsTree.impure(term, owner,
              generateShortcutIf(term, refSym, yCps.transformed, methodSymbol, true), internalKind2))
          )
       case other =>
  ...
  def generateShortcutIf(origin: Tree, x: Tree, y:Tree, methodSymbol: Symbol, asyncConstant:Boolean)(using Context, CpsTopLevelContext): Tree = {
