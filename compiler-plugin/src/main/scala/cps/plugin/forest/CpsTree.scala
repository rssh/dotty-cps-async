...
sealed trait CpsTree {
      ...
      case AsyncKind.Async(_) =>
        next.asyncKind match
          case AsyncKind.Sync | _: AsyncKind.AsyncLambda =>
            MapCpsTree(EmptyTree, owner, this, MapCpsTreeArgument(None, next))
          case AsyncKind.Async(_) =>
            FlatMapCpsTree(EmptyTree, owner, this, FlatMapCpsTreeArgument(None, next))
  ...
  /** @returns
    *   a copy of cpstree with origin set to origin. TODO: remove
    */
  def withOrigin(term: Tree): CpsTree

  def changeOwner(newOwner: Symbol)(using Context): CpsTree

  /** apply runtime await. If this CpsTree is adync lambda f: x1...xn => F[y] transform one to f' =
    * summon[RuntiemAwait[F].await(f(x1..xN))(monad,monad-context)] and return CpsTree with f' in unpure otherwise - unchanged or
    * throw error TODO: move to Lambda
    *
...
case class PureCpsTree(
  ...
  override def changeOwner(newOwner: Symbol)(using Context) =
    ...
    else PureCpsTree(origin.changeOwner(owner, newOwner), newOwner, term.changeOwner(owner, newOwner))
...
case class SeqCpsTree(
  ...
  override def normalizeAsyncKind(using Context, CpsTopLevelContext): CpsTree = {
    ...
    if (normalizedLast eq last) then this
    else SeqCpsTree(origin, owner, prevs, normalizedLast)
  }
  ...
  override def unpure(using Context, CpsTopLevelContext) = {
    if (prevs.isEmpty) then last.unpure
    else
  ...
  override def transformed(using Context, CpsTopLevelContext): Tree = {
    if (prevs.isEmpty) then last.transformed
    else
      asyncKind match
        case AsyncKind.AsyncLambda(_) =>
...
case class AsyncTermCpsTree(
  ...
  override def castOriginType(ntpe: Type)(using Context, CpsTopLevelContext): CpsTree = {
    if (origin.tpe =:= ntpe) then this
    else typed(Typed(origin, TypeTree(ntpe)).withSpan(origin.span))
  }
...
case class MapCpsTree(
...
)(using Context, CpsTopLevelContext)
    extends AsyncCpsTree {
  ...
  override def castOriginType(ntpe: Type)(using Context, CpsTopLevelContext): CpsTree = {
    if (ntpe =:= originType) then this
    else copy(mapFun = mapFun.copy(body = mapFun.body.castOriginType(ntpe)))
  }
  ...
  override def normalizeAsyncKind(using Context, CpsTopLevelContext): CpsTree = {
    ...
    if (normalizedSource eq mapSource) then this
    else MapCpsTree(origin, owner, normalizedSource, mapFun)
  }
  ...
  override def appendInBlock(next: CpsTree)(using Context, CpsTopLevelContext): CpsTree = {
    next.asyncKind match
      case AsyncKind.Async(_) =>
        FlatMapCpsTree(origin, owner, mapSource, FlatMapCpsTreeArgument(mapFun.optParam, mapFun.body.appendInBlock(next)))
      case _ =>
        MapCpsTree(origin, owner, mapSource, MapCpsTreeArgument(mapFun.optParam, mapFun.body.appendInBlock(next)))
  }
    ...
    body.unpure match
      ...
      case Some(syncBody) =>
        ...
        val param = optParam match
          ...
          case None =>
            val sym =
              newSymbol(owner, "_unused".toTermName, Flags.EmptyFlags, mapCpsTree.mapSource.originType.widen, Symbols.NoSymbol)
...
case class FlatMapCpsTree(
  ...
  override def normalizeAsyncKind(using Context, CpsTopLevelContext): CpsTree = {
    ...
    if ((normalizedSource eq flatMapSource) && (normalizedBody eq flatMapFun.body)) then this
    else FlatMapCpsTree(origin, owner, normalizedSource, FlatMapCpsTreeArgument(flatMapFun.optParam, normalizedBody))
  }
  ...
  override def appendInBlock(next: CpsTree)(using Context, CpsTopLevelContext): CpsTree = {
    FlatMapCpsTree(origin, owner, flatMapSource, FlatMapCpsTreeArgument(flatMapFun.optParam, flatMapFun.body.appendInBlock(next)))
  }
...
case class FlatMapCpsTreeArgument(
  ...
  def makeLambda(cpsTree: FlatMapCpsTree)(using Context, CpsTopLevelContext): Block = {
    ...
    val param = optParam.getOrElse {
      val sym = newSymbol(owner, "_unused".toTermName, Flags.EmptyFlags, cpsTree.flatMapSource.originType.widen, Symbols.NoSymbol)
      ValDef(sym, EmptyTree)
    }
...
case class LambdaCpsTree(
  ...
  override def castOriginType(ntpe: Type)(using Context, CpsTopLevelContext): CpsTree =
      ...
      ntpe match
        ...
        case _ =>
          throw CpsTransformException(
            s"Can't cast lambda to type ${ntpe.show}: expected AppliedType but have ${ntpe}",
            origin.srcPos
          )
    ...
    else throw CpsTransformException(s"Can't cast lambda to type ${ntpe.show}: expected function type", origin.srcPos)
  ...
  override def normalizeAsyncKind(using Context, CpsTopLevelContext): CpsTree = {
    ...
    if (normalizedBody eq cpsBody) then this
    else LambdaCpsTree(origin, owner, originDefDef, originClosureType, normalizedBody)
  }
        ...
        origin match
          ...
          case _ =>
            ...
            val closure = Closure(
              ...
              { tss =>
                TransformUtil
                  .substParams(unpureBody, originParams, tss.head)
...
case class OpaqueAsyncLambdaTermCpsTree(
  ...
  override def castOriginType(ntpe: Type)(using Context, CpsTopLevelContext): CpsTree = {
    if (origin.tpe =:= ntpe) then this
    else typed(Typed(origin, TypeTree(ntpe)))
  }
          ...
          cntArgs match
            ...
            case other =>
              if (optParamType.isDefined) {
                ...
                throw CpsTransformException(
                  s"Can't unwrapped base type of ${cntBase.show} in the reuslt of ${tpe.show} ",
                  origin.srcPos
                )
              }
        case other =>
          throw CpsTransformException(
            s"Expected that result type of AsyncLambda(${tpe.show}) is AppliedType, we have ",
            origin.srcPos
          )
...
/** one Unit
  */
case class UnitCpsTree(override val origin: Tree, override val owner: Symbol) extends SyncCpsTree {
...
case class BlockBoundsCpsTree(internal: CpsTree) extends CpsTree {
  ...
  override def normalizeAsyncKind(using Context, CpsTopLevelContext): CpsTree = {
    ...
    if (normalizedInternal eq internal) then this
...
case class SelectTypeApplyTypedCpsTree(
    records: Seq[SelectTypeApplyTypedCpsTree.Operation],
    nested: CpsTree,
    override val origin: Tree
) extends CpsTree {
  ...
  override def normalizeAsyncKind(using Context, CpsTopLevelContext): CpsTree = {
    ...
    if (normalizedNested eq nested) then this
    else SelectTypeApplyTypedCpsTree(records, normalizedNested, origin)
  }

  override def originType(using Context): Type =
    if (records.isEmpty) then nested.originType
    else records.last.originType

  override def castOriginType(ntpe: Type)(using Context, CpsTopLevelContext): CpsTree = {
    if (ntpe =:= originType) then this
    else typed(Typed(EmptyTree, TypeTree(ntpe)))
  }
    ...
    val retval = nested.asyncKind match
      ...
      case kind @ AsyncKind.Async(internalKind) =>
        internalKind match
          case AsyncKind.Sync =>
            val paramSym =
              newSymbol(owner, "xSelectTypeApplyCpsTree".toTermName, Flags.EmptyFlags, nested.originType.widen, Symbols.NoSymbol)
          ...
          case _ =>
            throw new CpsTransformException(
              s"SelectTypeApplyTypedCpsTree.transformed: unexpected nested internal kind:${internalKind}",
              origin.srcPos
            )
...
object SelectTypeApplyTypedCpsTree {
    ...
    override def prefixTerm(term: Tree, kind: AsyncKind)(using Context, CpsTopLevelContext): Tree =
      ...
      kind match
        ...
        case AsyncKind.Async(internalKind) =>
          ...
          else Select(term, Types.TermRef(term.tpe, sym)).withSpan(origin.span)
        case AsyncKind.AsyncLambda(bodyKind) =>
  ...
  case class OpTypeApply(origin: TypeApply) extends Operation {
    ...
    override def prefixTerm(term: Tree, kind: AsyncKind)(using Context, CpsTopLevelContext): Tree = {
      if (origin.args.isEmpty) then term
      else TypeApply(term, origin.args).withSpan(origin.span)
    }
...
case class InlinedCpsTreeDisabled(
    override val origin: Inlined,
    override val owner: Symbol,
...
/** Definition cps tree, Can be only in block and can't be last in block. (TODO: checked by BlockBoundsCpsTree)
  ...
  * @param changed
  */
case class MemberDefCpsTree(override val origin: Tree, override val owner: Symbol, definition: MemberDef) extends CpsTree {
...
case class CallChainSubstCpsTree(override val origin: Tree, override val owner: Symbol, call: CpsTree) extends CpsTree {
      ...
      finishChainSelect.asyncKind match
        ...
        case AsyncKind.Async(internalKind) =>
          ...
          FlatMapCpsTree(
            ...
            owner,
            finishChainSelect,
            FlatMapCpsTreeArgument(Some(nValDef), CpsTree.impure(origin, owner, nValRef, internalKind))
          )
        case AsyncKind.AsyncLambda(bodyKind) =>
