...
object ValDefTransform {
  ...
  def apply(term: ValDef, owner: Symbol, nesting: Int)(using Context, CpsTopLevelContext): CpsTree = {
    ...
    if (term.rhs.isEmpty) then throw CpsTransformException(s"ValDef without right part: $term", term.srcPos)
    val cpsRhs = RootTransform(term.rhs, term.symbol, nesting + 1)(using ctx.withOwner(term.symbol), tctx)

    cpsRhs.asyncKind match
      case AsyncKind.Sync =>
        if (cpsRhs.isOriginEqSync) then CpsTree.unchangedPure(term, owner)
        else
      ...
      case AsyncKind.Async(_) =>
        val mapCpsTreeArgument = if (true && term.symbol.flags.is(Flags.Mutable)) then
          val paramSym =
            Symbols.newSymbol(owner, ("p" + term.name).toTermName, Flags.EmptyFlags, term.tpt.tpe.widen, coord = term.symbol.coord)
  ...
  def applyMemoization(cpsRhs: CpsTree, owner: Symbol, memoization: Tree, term: ValDef)(using
      Context,
      CpsTopLevelContext
  ): CpsTree = {
    if (memoization.tpe <:< Symbols.requiredClassRef("cps.CpsMonadMemoization.Default").appliedTo(Types.WildcardType)) then cpsRhs
          ...
          cpsRhs.asyncKind match
            case AsyncKind.Sync =>
              throw CpsTransformException(
                s"Sync value ${term.name} used in both sync and async way with automatic coloring",
                term.srcPos
              )
            case AsyncKind.Async(internalKind) =>
              //  monad.map[F[T],F[T]](cpsRhs)(x => memoization.apply(x))
              val toMemoizeSym = Symbols.newSymbol(
                owner,
                (term.name.toString + "$toMemoize").toTermName,
                Flags.EmptyFlags,
                term.tpt.tpe.widen,
                coord = term.symbol.coord
              )
        ...
        case None =>
          cpsRhs.asyncKind match
            ...
            case AsyncKind.Async(internalKind) =>
              ...
              val toMemoizeSym =
                Symbols.newSymbol(owner, "toMemoize".toTermName, Flags.EmptyFlags, term.tpt.tpe.widen, coord = term.symbol.coord)
      ...
      val dynamicApType =
        AppliedType(Symbols.requiredClassRef("cps.CpsMonadMemoization.DynamicAp"), List(summon[CpsTopLevelContext].monadType, et))
              ...
              cpsRhs.asyncKind match
                ...
                case AsyncKind.Async(internalKind) =>
                  ...
                  FlatMapCpsTree(
                    ...
                    FlatMapCpsTreeArgument(
                      Some(toMemoizeValDef),
                      CpsTree.impure(term, owner, Apply(Select(mm, "apply".toTermName), List(toMemoizeRef)), AsyncKind.Sync)
                    )
                  )
                case AsyncKind.AsyncLambda(bodyKind) =>
    ...
    else throw CpsTransformException(s"Unknown memoization type ${memoization.tpe}", term.srcPos)
  }
