...
object ApplyArg {
        ...
        case inlined @ Inlined(call, bindings, expansion) =>
          expansion match
            case CheckRepeated(internal, elemtpt) =>
              Some(
                (
                  SeqLiteralMbInlined(enclosingInlined = inlined +: internal.enclosingInlined, seqLiteral = internal.seqLiteral),
                  elemtpt
                )
              )
            case _ => None
  ...
  def apply(
      ...
      paramType: Type,
      isByName: Boolean,
      isDirectContext: Boolean,
      owner: Symbol,
      dependFromLeft: Boolean,
      named: Option[TermName],
      nesting: Int
  )(using Context, CpsTopLevelContext): ApplyArg = {
    ...
    val retval = expr match
      ...
      case _ =>
        ...
        if (isByName) then ByNameApplyArg(paramName, paramType, cpsExpr, isDirectContext, named)
        else
          paramType match
...
case class PlainApplyArg(
  ...
  /** Output the expression inside call this can
    *
    * //Are we change symbol when do changeOwner to tree ? If yes, we should be extremally careful with different refs to
    * optIdentSym. Maybe better do expr function from sym ?
    *
    * TODO: dependFromLeft
    */
  override def exprInCallNotNamed(callMode: ApplyArgCallMode, optRuntimeAwait: Option[Tree])(using
      Context,
      CpsTopLevelContext
  ): Tree =
    import AsyncKind.*
    expr.asyncKind match
      ...
      case AsyncLambda(internal) =>
        if (callMode == ApplyArgCallMode.ASYNC_SHIFT) then expr.transformed
        else
          expr.unpure match
            case Some(tree) => tree
            case None =>
              if (lambdaCanBeUnshifted) then unshiftLambdaAsTree
              else
                optRuntimeAwait match
                  ...
                  case None =>
                    throw CpsTransformException(
                      s"Can't transform function (both shioft and runtime-awaif for ${summon[CpsTopLevelContext].monadType} are not found)",
                      expr.origin.srcPos
                    )
  ...
  def unshiftLambda(expr: CpsTree)(using Context, CpsTopLevelContext): CpsTree = {
    expr match
      case LambdaCpsTree(origin, owner, originDefDef, closureType, cpsBody) =>
        ...
        val nCpsBodyTree = ctx.typer.typed(
          untpd.Apply(
  ...
  def unshiftLambdaAsTree(using Context, CpsTopLevelContext): Tree = {
    ...
    def applyFlatten(tree: Tree): Tree = {
      val tctx = summon[CpsTopLevelContext]
      val monadRef = tctx.cpsMonadRef
      val nTree = summon[Context].typer
        .typed(
          untpd.Apply(
            untpd.Select(untpd.TypedSplice(monadRef), "flatten".toTermName),
    ...
    callMode match
      ...
      case _ =>
        expr.unpure match
          case Some(tree) => tree
          case None =>
            if (lambdaCanBeUnshifted) then unshiftLambdaTree
            else
              optRuntimeAwait match
                case Some(runtimeAwait) =>
