...
class SLSelectLoop[F[_]](using val am:CpsMonad[F]):
  ...
  def fold1_async[S](s0:S)(step: (S,SLSelectLoop[F])=> F[S|SLSelectLoop.Done[S]], limit: Int): F[S] = {
     if (limit > 0) then
       am.flatMap(step(s0, new SLSelectLoop[F])){ r =>
          r match
...
object SLSelectLoop:
  ...
  def loopImpl[F[_]:Type](pf: Expr[PartialFunction[Any,Boolean]], m: Expr[CpsMonad[F]], mc:Expr[CpsMonadContext[F]])(using Quotes): Expr[Unit] =
      ...
      def builder(caseDefs: List[SelectorCaseExpr[F]])(using Quotes):Expr[Unit] = {
          def s0(using Quotes) = '{
            new SLSelectLoop[F](using $m)
          }
          def g(using Quotes): Expr[SLSelectLoop[F]] = caseDefs.foldLeft(s0) { (s, e) =>
            e.appended(s)
          }
  ...
  def runImpl[F[_]:Type, C:Type, A:Type,B :Type](builder: List[SelectorCaseExpr[F]]=>Expr[B],
  ...
  def runImplTree[F[_]:Type, A:Type, B:Type](using Quotes)(
    ...
    val retval = pf match
      case Lambda(valDefs, body) =>
        runImplTree[F,A,B](builder, body.changeOwner(Symbol.spliceOwner), m)
    ...
    if (DEBUG) {
      TransformUtil.findDefinitionWithoutSymbol(retval.asTerm) match
        case Some(d) =>
          println(s"runImpl: definition without symbol ${d.show}")
        case None =>
          println(s"runImpl: all definitions have symbols")
    }
    retval.asExprOf[B]
  ...
  def parseCaseDef[F[_]:Type,S:Type](using qctx: Quotes)(caseDef: quotes.reflect.CaseDef, m:Expr[CpsMonad[F]]): SelectorCaseExpr[F] =
    ...
    caseDef.pattern match 
      case Inlined(_,List(),body) => 
            parseCaseDef[F,S](CaseDef(body, caseDef.guard, caseDef.rhs),m)
