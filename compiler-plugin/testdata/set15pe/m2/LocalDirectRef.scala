...
object localDirectRef {
  ...
  extension (p:PureEffect.type) {
    ...
    def localDirectRef[T<:AnyRef](t:T)(using CpsDirect[PureEffect]): DirectRef[T] =
      DirectRef[T](await(PERef.make(t)))

    def localDirectIntRef(t:Int)(using CpsDirect[PureEffect]): DirectIntRef =
