...
@experimental
class TestUseResourceFromPlugin {
  ...
  def doSomethingWithResouce(using CpsDirect[[X]=>>Resource[PureEffect,X]]): PEToyLogger = {
      val logger = PEToyLogger.make()
      val r1 = openResourceDirect("r1", logger)
      logger.log("inside")
      logger
  }

 @Test
 def testUseResourceM4(): Unit = {
    val c = async[[X]=>>Resource[PureEffect,X]]{
      doSomethingWithResouce
    }
