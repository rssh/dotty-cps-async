...
object Listener:
  ...
  inline def acceptingListener[T](inline consumer: (T, Async.Source[T]) => Unit) =
    new Listener[T]:
      val lock = null
      def complete(data: T, source: Async.Source[T]) = consumer(data, source)

  /** Returns a simple [[Listener]] that always accepts the item and sends it to the consumer. */
  inline def apply[T](consumer: (T, Async.Source[T]) => Unit): Listener[T] = acceptingListener(consumer)
