...
object AsyncList {
  ...
  case class Wait[F[_]: CpsConcurrentMonad, T](fs: F[AsyncList[F, T]]) extends AsyncList[F, T]:
    ...
    def takeTo[B <: Growable[T]](buffer: B, n: Int): F[B] =
      if n == 0 then summon[CpsMonad[F]].pure(buffer)
      else summon[CpsMonad[F]].flatMap(fs)(_.takeTo(buffer, n))
  ...
  case class Cons[F[_]: CpsConcurrentMonad, T](head: T, tailFun: () => AsyncList[F, T]) extends AsyncList[F, T]:
    ...
    def mapAsync[S](f: T => F[S]): AsyncList[F, S] =
      Wait(
        summon[CpsMonad[F]].map(f(head))(h => Cons(h, () => tailFun().mapAsync(f)))
      )
    ...
    def filter(p: T => Boolean): AsyncList[F, T] =
      if p(head) then Cons(head, () => tailFun().filter(p))
      else tailFun().filter(p)

    def filterAsync(p: T => F[Boolean]): AsyncList[F, T] =
      Wait(summon[CpsAsyncMonad[F]].map(p(head)) { c =>
        if c then Cons(head, () => tailFun().filterAsync(p))
        else tailFun().filterAsync(p)
      })

    def find(p: T => Boolean): F[Option[T]] =
      if p(head) then summon[CpsMonad[F]].pure(Some(head))
      else tailFun().find(p)

    def findAsync(p: T => F[Boolean]): F[Option[T]] =
      summon[CpsMonad[F]].flatMap(p(head)) { c =>
        if c then summon[CpsMonad[F]].pure(Some(head))
        else tailFun().findAsync(p)
      }
  ...
  ): CpsAsyncEmitAbsorber4[AsyncList[F, T], F, C, T] =
...
class AsyncListEmitAbsorber[F[_], C <: CpsMonadContext[F], T](using ec: ExecutionContext, auxMonad: CpsConcurrentMonad.Aux[F, C])
    extends BaseUnfoldCpsAsyncEmitAbsorber[AsyncList[F, T], F, C, T]:
