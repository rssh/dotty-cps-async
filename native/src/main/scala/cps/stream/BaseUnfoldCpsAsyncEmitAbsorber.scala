...
trait BaseUnfoldCpsAsyncEmitAbsorber[R, F[_], C <: CpsMonadContext[F], T](using
  ...
  class State:
    ...
    private def queueConsumer(): F[SupplyEventRecord] =
      if (finishRef.get() != null) then asyncMonad.pure(Finished(finishRef.get()))
      else
    ...
    def consume(): F[SupplyEventRecord] =
      this.synchronized {
        if (emitStart.compareAndSet(false, true)) then asyncMonad.pure(SpawnEmitter)
        else
  ...
  def evalAsyncInContext(f: CpsAsyncEmitter[F, T] => F[Unit]): R =
    ...
    def step(state: State): F[Option[(T, State)]] =
      ...
      if r eq null then asyncMonad.flatMap(nextEvent())(e => handleEvent(e))
      else
        r match
