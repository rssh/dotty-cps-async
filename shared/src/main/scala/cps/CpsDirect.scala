...
/** Direct style context marker, used to mark function, whritten in direct style. When function have given parameter of type
  * CpsDirect[F] or return context lambda with CpsDirect[F] parameter, then compiler plugin will transform function result into
