...
object Async {
  ...
  /** transform expression within given monad. Use this function is you need to force async-transform from other macros.
    */
  def transformMonad[F[_]: Type, T: Type, C <: CpsMonadContext[F]: Type](f: Expr[T], dm: Expr[CpsMonad[F]], mc: Expr[C])(using
      Quotes
  ): Expr[F[T]] =
    import quotes.reflect._
    ...
    try
      ...
      val r = WithOptExprProxy("cpsMonad", dm) { dm =>
        ...
        if (
          ...
          if (dm.asTerm.tpe <:< TypeRepr.of[CpsAsyncMonad[F]]) then
            ...
            val transformed =
              loomTransform[F, T, C & CpsTryMonadContext[F]](f, dma, mc.asExprOf[C & CpsTryMonadContext[F]], cpsRuntimeAwait, flags)
      ...
      if (flags.printCode)
        try println(s"transformed value: ${r.show}")
        catch
          case ex: Exception =>
  ...
  def rootTransform[F[_]: Type, T: Type, C <: CpsMonadContext[F]: Type](
      f: Expr[T],
      dm: Expr[CpsMonad[F]],
      mc: Expr[C],
    ...
    import quotes.reflect._
    val cpsCtx = TransformationContext[F, T, C](f, tType, dm, mc, optRuntimeAwait, optRuntimeAwaitProvider, flags, nesting, parent)
    val retval = f match
  ...
  def nestTransform[F[_]: Type, T: Type, C <: CpsMonadContext[F]: Type, S: Type](
      f: Expr[S],
  ...
  def transformContextInstanceMonad[F[_]: Type, T: Type, C <: CpsTryMonadInstanceContext[F]: Type](f: Expr[T], dm: Expr[C])(using
      Quotes
  ): Expr[F[T]] =
    '{
  ...
  def loomTransform[F[_]: Type, T: Type, C <: CpsTryMonadContext[F]: Type](
      f: Expr[T],
