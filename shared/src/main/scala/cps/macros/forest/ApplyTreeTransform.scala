...
trait ApplyTreeTransform[F[_], CT, CC <: CpsMonadContext[F]]:
  ...
  def runApply(applyTerm: Apply, fun: Term, args: List[Term], tails: List[Seq[ApplyArgRecord]])(owner: Symbol): CpsTree =
    ...
    val r = fun match
      ...
      case applyFun @ Apply(TypeApply(obj2, targs2), args1)
          if obj2.symbol == nonLocalReturnsThrowReturnSym /*&& cpsCtx.inShiftedReturning*/ =>
      ...
      case List(y) =>
        ...
        (xCpsTree.syncOrigin, yCpsTree.syncOrigin) match
          case (Some(xx), Some(yy)) =>
            if (!xCpsTree.isChanged && !yCpsTree.isChanged) then CpsTree.pure(owner, applyTerm, false)
            else
              CpsTree.pure(
                owner,
          ...
          case (None, None) =>
            xCpsTree.monadFlatMap(
              { t =>
                val (tOp, tCnt) = shortcutOpConst(t)
                genShortcutIf(tOp.asExprOf[Boolean], tCnt, yCpsTree.transformed.asExprOf[F[Boolean]]).asTerm
              },
              TypeRepr.of[Boolean]
            )
      case _ =>
    ...
    obj match {
      case Select(obj1, method) =>
        ...
        if (cpsObj1.isAsync)
          ...
          val cpsObj = cpsObj1.select(obj, obj.symbol, obj.tpe).typeApply(fun, targs, fun.tpe)
          handleArgs1(applyTerm, fun, cpsObj, args, tails)(owner)
        else handleArgs1(applyTerm, fun, CpsTree.pure(owner, fun), args, tails)(owner)
      case Ident(name) =>
        ...
        cpsObj match
          ...
          case cls: CallChainSubstCpsTree =>
            ...
            sameSelect(cls.shifted, methodName, List.empty, args)(owner) match
              case None =>
                if (cpsCtx.flags.debugLevel >= 15) then cpsCtx.log(s"not found name ${methodName} for ${cls.shifted.show}")
                val cpsObj1 = cpsObj.select(fun, fun.symbol, fun.tpe)
                handleArgs1(applyTerm, fun, cpsObj1, args, tails)(owner)
              case Some(term) =>
                if (cpsCtx.flags.debugLevel >= 15) then cpsCtx.log(s"found sameSelect: ${term.show} ")
  ...
  def handleFunIdent(applyTerm: Apply, fun: Term, args: List[Term], name: String, tails: List[Seq[ApplyArgRecord]])(
      owner: Symbol
  ): CpsTree =
    handleArgs1(applyTerm, fun, CpsTree.pure(owner, fun), args, tails)(owner)
  ...
  def shiftedLambdaType(tpe: TypeRepr): TypeRepr =
    tpe.widen match {
      case MethodType(paramNames, paramTypes, resType) =>
        ...
        MethodType(paramNames)(mt => paramTypes, mt => TypeRepr.of[F].appliedTo(resType))
      case PolyType(paramNames, paramBounds, resType) =>
        PolyType(paramNames)(pt => paramBounds, pt => TypeRepr.of[F].appliedTo(resType))
  ...
  def handleArgs1(
    ...
    if (callCpsDirect && !fun.symbol.flags.is(Flags.Inline)) then
      ...
      if (
          !funSym.isDefinedInCurrentRun &&
          !funSym.flags.is(Flags.Inline) &&
          !funSym.hasAnnotation(cpsTransformedSymbol)
        )
      then
        ...
        throw MacroError(
          s"Function ${fun.symbol} is cps-=direct function compiled without dotty-cps-async-compiler plugin",
          applyTerm.asExpr
        )
    if cpsCtx.flags.debugLevel >= 15 then
    ...
    if (!existsAsyncArg && !existsShiftedLambda && !shouldBeChangedSync) {
      ...
      cpsFun match
        case lt: AsyncLambdaCpsTree =>
          if (callCpsDirect) then throw MacroError("Lambda accepting CpsDirect is not supported yet", applyTerm.asExpr)
          else CpsTree.impure(owner, Select.unique(lt.rLambda, "apply").appliedToArgss(args :: tailArgss), applyTerm.tpe)
        case _ =>
          ...
          if (!cpsFun.isAsync) then
            ...
            cpsFun match
              case sel: SelectTypeApplyCpsTree =>
                ...
                sel.nested.syncOrigin match
                  ...
                  case None =>
                    ...
                    sel.nested.monadFlatMap(
                      { x =>
                        val nFun = sel.apply(x)
                        val nCpsFun = CpsTree.pure(owner, nFun, true)
                        val cpsCall =
                          buildApplyPrependArgsFlatMaps(nCpsFun, nFun, applyRecords, applyTerm, argsProperties, unpure, tails)(
                            owner
                          )
                        cpsCall.transformed
                      },
              ...
              case _ =>
                ...
                val cpsCallWithArgs =
                  buildApplyPrependArgsFlatMaps(nCpsFun, fun, applyRecords, applyTerm, argsProperties, unpure, tails)(owner)
      ...
      if (cpsCtx.flags.debugLevel >= 15) then cpsCtx.log(s"handleArgs1: result = ${retval}")
      retval
    }
  }

  def optWrapCallIntoCpsDirect(originApplyTerm: Apply, optCpsDirectArg: Option[Term], call: Term, isChanged: Boolean)(
      owner: Symbol
  ): CpsTree =
  ...
  def wrapCallIntoCpsDirect(origin: Term, cpsDirectArg: Term, applyCall: Term)(owner: Symbol): CpsTree = {
    cpsDirectArg match
      case Apply(TypeApply(byInclusionCn, List(tf, tg)), List(fctx, fgincl))
        ...
        if (tf.tpe =:= tg.tpe) then
          ...
          val adoptedTree = Apply.copy(origin)(
            TypeApply(Ref(adoptCpsedCallCompileTimeOnlySymbol), List(TypeTree.of[F], Inferred(origin.tpe.widen))),
          ...
          val lambda = Lambda(
            ...
            { (owner, params) =>
              ...
              val adoptedTree = Apply.copy(origin)(
                TypeApply(Ref(adoptCpsedCallCompileTimeOnlySymbol), List(tg, Inferred(origin.tpe.widen))),
                List(substituteCpsDirectArgRequired(tree, cpsDirectArg, nCpsDirectArg))
              )
      ...
      case _ =>
        val tree = applyCall
        val adoptedTree = Apply.copy(origin)(
          TypeApply(Ref(adoptCpsedCallCompileTimeOnlySymbol), List(Inferred(TypeRepr.of[F]), Inferred(origin.tpe.widen))),
          List(tree)
        )
        CpsTree.impure(owner, adoptedTree, origin.tpe.widen)
  }
  ...
  def buildApplyPrependArgsFlatMaps(
      cpsFun: CpsTree,
      fun: Term,
    ...
    val lastCpsTree: CpsTree = if (!argsProperties.usePrepend && cpsFun.isSync) {
      runFold = false
      if (
        !argsProperties.hasShiftedLambda && !cpsFun.isChanged &&
  ...
  def shiftedApplyCps(
      ...
      applyTerm: Apply,
      argsSummaryProperties: ApplyArgsSummaryProperties
  )(owner: Symbol): CpsTree =
              ...
              cpsCtx.runtimeAwaitProvider match
                case Some(runtimeAwaitProvider) =>
                  ...
                  val call = Apply.copy(applyTerm)(
                    Apply(
                      TypeApply(
                        Select.unique(runtimeAwaitProvider.asTerm, "withRuntimeAwait"),
        ...
        (funCpsTree: @nowarn("msg=Unreachable case .*")) match
          ...
          case SelectTypeApplyCpsTree(optOrigin, nested, targs, selects, otpe, changed) =>
            if (selects.isEmpty) then
              if (targs.isEmpty) then shiftedApplyCps(nested, argRecords, argTails, applyTerm, argsSummaryProperties)(owner)
              else
          ...
          case BlockCpsTree(owner, stats, last) =>
            BlockCpsTree(owner, stats, shiftedApplyCps(last, argRecords, argTails, applyTerm, argsSummaryProperties)(owner))
          ...
          case AppendCpsTree(frs, snd) =>
            AppendCpsTree(frs, shiftedApplyCps(snd, argRecords, argTails, applyTerm, argsSummaryProperties)(owner))
  ...
  def shiftedApplyTerm(
    ...
    def applyCpsDeferredShift(): PartialShiftedApply =
      PartialShiftedApply(
        ApplicationShiftType.CPS_DEFERR_TO_PLUGIN,
    ...
    def checkInplaceAsyncMethodCandidate(methodSym: Symbol, qual: Term, targs: List[TypeTree]): Either[MessageWithPos, Boolean] =
      ...
      if (paramSymss.isEmpty) then Left(MessageWithPos(s"${methodSym.name} for qual=${qual} should have arguments", mPos))
      else
    ...
    def findInplaceAsyncMethodCall(
        qual: Term,
        shiftedName: String,
        targs: List[TypeTree],
        pos: Position
      ...
      qual.tpe.typeSymbol.methodMember(shiftedName) match
        ...
        case overloaded =>
          ...
          if (foundDirect.isEmpty) then
            if (foundUseExtra.isEmpty) then Left(errors)
            else
              // TODO: add retval to overload check
              Right(
                applyCpsOnlyShift(args =>
                  Select.overloaded(qual, shiftedName, TypeTree.of[F].tpe :: targs.map(_.tpe), monad :: args)
                )
              )
          else if (foundUseExtra.isEmpty) then
            Right(applyCpsOnlyShift(args => Select.overloaded(qual, shiftedName, targs.map(_.tpe), args)))
          else
    ...
    def shiftSelectTypeApplyApply(x: Select, targs: List[TypeTree]): PartialShiftedApply =
      ...
      x.qualifier match
        case qual @ Apply(Select(col, "withFilter"), List(predicate))
            if (qual.tpe <:< TypeRepr.of[scala.collection.WithFilter[?, ?]]) =>
          ...
          colAsyncShiftSearch match
            ...
            case failure: ImplicitSearchFailure =>
              if (cpsCtx.runtimeAwait.isDefined || cpsCtx.runtimeAwaitProvider.isDefined) then applyCpsAwaitShift()
              else throw MacroError(s"Can't resolve [${askedShiftedType.show}] when parsing withFilter ", posExpr(col))
        case _ =>
            ...
            case Left(funErrors) =>
              ...
              asyncShiftSearch match
                  ...
                  shiftSymbol.methodMember(xName) match
                    ...
                    case other =>
                      applyCpsOnlyShift { args =>
                        ...
                        val shiftedCaller = Select.overloaded(
                          success2.tree,
                          xName,
                          (TypeTree.of[F] :: targs).map(_.tpe),
                          List(qual, monad),
                          expectedType
                        )
                ...
                case failure2: ImplicitSearchFailure =>
                  if (
                      x.symbol.isDefinedInCurrentRun &&
                      x.symbol.hasAnnotation(Symbol.requiredClass("cps.plugin.annotation.makeCPS"))
                    )
                  then
                  ...
                  else if (cpsCtx.runtimeAwait.isDefined || cpsCtx.runtimeAwaitProvider.isDefined) then applyCpsAwaitShift()
                  else
                    ...
                    throw MacroError(
                      s"Can't find AsyncShift (${failure2.explanation}) or async functions) for qual=${qual} name = ${xName}, shiftedName=${shiftedName}, askedShiftedType=${askedShiftedType.show}",
                      posExpr(x)
                    )
    ...
    funTerm match
      ...
      case Lambda(params, body) =>
        if (cpsCtx.runtimeAwait.isDefined || cpsCtx.runtimeAwaitProvider.isDefined) then applyCpsAwaitShift()
        else throw MacroError(s"Can't shift lambda ${funTerm.show} without runtime await", posExprs(funTerm))
  ...
  def buildApply(
    ...
    if (cpsCtx.flags.debugLevel >= 15)
      ...
      cpsCtx.log(
        s"buildApply: cpsFun.isSync=${cpsFun.isSync}, withShiftedLambda=${argsProperties.hasShiftedLambda}, inShiftedCallChain=${inShiftedCallChain}"
      )
    val origFun = fun
    val withAsync = argsProperties.hasAsync
    val applyTpe = applyTerm.tpe
    if (argsProperties.hasShiftedLambda) buildShiftedApply(cpsFun, fun, argRecords, argsProperties, tails, applyTerm)(owner)
    else
              ...
              if (inShiftedCallChain)
                ...
                argsProperties.cpsDirectArg match
                  case Some(cpsDirectArg) =>
                    cpsFun.monadFlatMap(
                      { x =>
                        wrapCallIntoCpsDirect(applyTerm, cpsDirectArg, x.appliedToArgss(argss))(owner).transformed
                      },
                      applyTpe
                    )
                  case None =>
  ...
  /** Shift the functional typr tpe. annotated ny makecps. If the function type does not contains hight-order function which can be
    * shifted - report error. (we call shift only for methods, annotated with makeCps) Note, that is is not the type of the shifted
    * methods (because it includes yet one type parameter F[_]), it is just the type which used to make typer happy with the shifted
    * arguments. This type will be throwed out on the later stage.
    * @param tpe
  ...
  def shiftTypeForCpsGeneratedMethod(tpe: TypeRepr, foundShiftedParams: Boolean, xPos: Term): TypeRepr = {
    ...
    val retval = tpe match
      case PolyType(names, bounds, res) =>
        ...
        val nRes = (np: PolyType) =>
          shiftTypeForCpsGeneratedMethod(res.substituteTypes(List(tpe.typeSymbol), List(np)), foundShiftedParams, xPos)
      ...
      case AppliedType(t, targs) =>
        ...
        if (tpe.isFunctionType || tpe.isContextFunctionType) then
          ...
          val nTargs = targs.zipWithIndex.map { (p, i) =>
            if (i == len - 1) then shiftTypeForCpsGeneratedMethod(p, shiftedParamFound, xPos)
            else if (!shiftedParamFound) then
              shiftedParamFound = isShiftedParam(p)
              if (shiftedParamFound) then shiftParamType(p, xPos)
              else p
            else shiftParamType(p, xPos)
          }
          if (tpe.isFunctionType) then t.appliedTo(nTargs)
          else defn.FunctionClass(len, false, false).typeRef.appliedTo(nTargs)
        else if (!foundShiftedParams) then
          report.errorAndAbort(s"@makeCps annotated method ${xPos} have no shifted args ", xPos.pos)
        else TypeRepr.of[F].appliedTo(tpe.widen)
      case _ =>
  ...
  def shiftParamType(tp: TypeRepr, xPos: Term): TypeRepr = {
    val retval = tp match
      case AppliedType(t, targs) =>
        if (tp.isFunctionType || tp.isContextFunctionType) then cpsTransformedType(tp)
        else tp
      case ByNameType(tp1) =>
      ...
      case AppliedType(funCn, params) =>
        if (tpe.isFunctionType || tpe.isContextFunctionType) then
          ...
          val nParams = params.zipWithIndex.map { (p, i) =>
            if (i == paramsLen - 1) then cpsTransformedType(p)
            else p
          }
  ...
  def run[F[_]: Type, T: Type, C <: CpsMonadContext[F]: Type](using
    ...
    class Bridge extends TreeTransformScope[F, T, C] {
      ...
      def bridge(): CpsExpr[F, T] =
        val treeResult = runApply(
          applyTerm.asInstanceOf[qctx.reflect.Apply],
