...
trait AsyncTreeShifter[F[_], CT, CC <: CpsMonadContext[F]]:
  ...
  def asyncShift(t: Term, shiftedSymbols: Set[Symbol]): Term =
    t match
      case Block(stats, last) => Block(stats.map(asyncShiftStatement(_, shiftedSymbols)), asyncShift(last, shiftedSymbols))
      case If(cond, thenp, elsep) =>
        If(asyncShift(cond, shiftedSymbols), asyncShift(thenp, shiftedSymbols), asyncShift(elsep, shiftedSymbols))
      case Match(scrutinee, cases) => Match(asyncShift(scrutinee, shiftedSymbols), cases.map(asyncShiftCaseDef(_, shiftedSymbols)))
      case While(cond, body)       => While(asyncShift(cond, shiftedSymbols), asyncShift(body, shiftedSymbols))
