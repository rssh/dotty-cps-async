...
object ConstTransform:
  ...
  def run[F[_]: Type, T: Type, C <: CpsMonadContext[F]: Type](using
      Quotes
  )(cpsCtx: TransformationContext[F, T, C], constTerm: quotes.reflect.Literal): CpsExpr[F, T] =
    import quotes.reflect._
    if (cpsCtx.flags.debugLevel >= 10) then cpsCtx.log(s"const: T=${Type.show[T]}, code=${cpsCtx.patternCode.show}")
