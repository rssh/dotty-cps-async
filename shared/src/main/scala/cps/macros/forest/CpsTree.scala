...
trait CpsTreeScope[F[_], CT, CC <: CpsMonadContext[F]] {
  ...
  sealed abstract class CpsTree:
    ...
    /** @return
      *   transformed term, which is result of transformation.
      */
    def transformed: Term

    /** @return
      *   Some(syncTerm) if this tree is sync,( syncTerm can-be origin term, can0be not). None if this tree is async.
      */
        ...
        case None =>
          try {
            ...
            val sealedTransformed =
              if (candidate.tpe <:< TypeRepr.of[F[T]]) then safeSealAs[F[T]](candidate)
  ...
  case class PureCpsTree(override val owner: Symbol, origin: qctx.reflect.Statement, isChanged: Boolean = false) extends CpsTree:
    ...
    override def inCake[F1[_], T1, C1 <: CpsMonadContext[F1]](otherCake: TreeTransformScope[F1, T1, C1]): otherCake.PureCpsTree =
      otherCake.PureCpsTree(otherCake.adoptSymbol(owner), otherCake.adoptStatement(origin), isChanged)
  ...
  case class AwaitSyncCpsTree(val owner: Symbol, val origin: Term, val otpe: TypeRepr) extends AsyncCpsTree:
    ...
    def castOtpe(newOtpe: TypeRepr): CpsTree =
      ...
      else monadMap(x => Typed(x, Inferred(newOtpe)), newOtpe)

    override def inCake[F1[_], T1, C1 <: CpsMonadContext[F1]](
        otherCake: TreeTransformScope[F1, T1, C1]
    ): otherCake.AwaitSyncCpsTree =
      otherCake.AwaitSyncCpsTree(otherCake.adoptSymbol(owner), otherCake.adopt(origin), otherCake.adoptType(otpe))
  ...
  case class MappedCpsTree(prev: CpsTree, op: Term => Term, otpe: TypeRepr) extends AsyncCpsTree:
    ...
    def transformed: Term = {
      ...
      val r = tmapTerm.appliedToArgss(
        List(
          List(prev.transformed.changeOwner(Symbol.spliceOwner)),
          List(
            Lambda(
              owner,
      ...
      val r = tpFlatMapTerm.appliedToArgss(
        List(
          ...
          List(
            Lambda(
              this.owner,
              MethodType(List("x"))(mt => List(wPrevOtpe), mt => TypeRepr.of[F].appliedTo(wOtpe)),
  ...
  // TODO: refactor
  //   add origin block
  case class BlockCpsTree(override val owner: Symbol, prevs: Queue[Statement], last: CpsTree) extends CpsTree:
    ...
    override def syncOrigin: Option[Term] =
      if prevs.isEmpty then last.syncOrigin
      else last.syncOrigin map (l => Block(prevs.toList, l).changeOwner(owner))
  ...
  case class InlinedCpsTree(owner: Symbol, origin: Inlined, bindings: List[Definition], nested: CpsTree) extends CpsTree:
    ...
    override def changeOwner(newOwner: Symbol): InlinedCpsTree = {
      if (newOwner eq owner) then this
      else InlinedCpsTree(newOwner, origin, bindings.map(_.changeOwner(newOwner)), nested.changeOwner(newOwner))
    }

  end InlinedCpsTree

  case class ValCpsTree(owner: Symbol, valDef: ValDef, rightPart: CpsTree, nested: CpsTree, canBeLambda: Boolean = false)
      extends CpsTree:
    ...
    def appendValDef(right: Term): Term =
      val nValDef = ValDef.copy(valDef)(name = valDef.name, tpt = valDef.tpt, rhs = Some(right.changeOwner(valDef.symbol)))
    ...
    def inCake[F1[_], T1, C1 <: CpsMonadContext[F1]](otherScope: TreeTransformScope[F1, T1, C1]): otherScope.ValCpsTree =
      otherScope.ValCpsTree(
        owner.asInstanceOf[otherScope.qctx.reflect.Symbol],
    ...
    def changeOwner(newOwner: Symbol): ValCpsTree =
      if (newOwner eq owner) then this
      else ValCpsTree(newOwner, valDef.changeOwner(newOwner), rightPart, nested.changeOwner(owner), canBeLambda)

  end ValCpsTree

  /** append cps tree, which is frs and then snd. we use this representation instead Mapped/Flatmapped in cases, where we later can
    * apply await to append term and simplify tree instead wrapping awaited tree in extra flatMap
    */
  case class AppendCpsTree(frs: CpsTree, snd: CpsTree) extends CpsTree:
    ...
    override def inCake[F1[_], T1, C1 <: CpsMonadContext[F1]](
        otherScope: TreeTransformScope[F1, T1, C1]
    ): otherScope.AppendCpsTree =
    ...
    override def changeOwner(newOwner: Symbol): AppendCpsTree =
      if (newOwner eq owner) then this
      else AppendCpsTree(frs.changeOwner(newOwner), snd)

  end AppendCpsTree

  case class AsyncLambdaCpsTree(owner: Symbol, originLambda: Term, params: List[ValDef], body: CpsTree, otpe: TypeRepr)
      extends CpsTree:
    ...
    override def rtpe =
      ...
      else throw MacroError("Sorry, functions with more than 3 parameters are not supported yet", posExprs(originLambda))
    ...
    override def changeOwner(newOwner: Symbol): AsyncLambdaCpsTree = {
      if (newOwner eq owner) then this
      else AsyncLambdaCpsTree(newOwner, originLambda, params, body, otpe)
    }

  end AsyncLambdaCpsTree

  /** when we have swhifted function, which should return F[A] but we want to have in F[A] methods with special meaning, which
    * should be performed on F[_] before jumping into monad (exampe: Iterable.withFilter) we will catch in ApplyTree such methods
    * and substitute to appropriative calls of shifted.
    */
  case class CallChainSubstCpsTree(owner: Symbol, origin: Term, shifted: Term, override val otpe: TypeRepr) extends CpsTree:
    ...
    override def changeOwner(newOwner: Symbol): CallChainSubstCpsTree =
      if (newOwner eq owner) then this
      else CallChainSubstCpsTree(newOwner, origin.changeOwner(newOwner), shifted.changeOwner(newOwner), otpe)
  ...
  /** represent select expression, which can be in monad or outside monad.
    *
    * constructor is not devoted to used directly: use SelectTypeApplyCpsTree.create instead. selects is reversed (i.e. external
    * added to head)
    */
  case class SelectTypeApplyCpsTree(
    ...
    override def changeOwner(newOwner: Symbol): SelectTypeApplyCpsTree = {
      if (newOwner eq owner) then this
      else SelectTypeApplyCpsTree(optOrigin, nested.changeOwner(newOwner), targs, selects, otpe, true)
    }
    ...
    override def transformed: Term =
      nested match
        ...
        case AwaitSyncCpsTree(nestOwner, nestOrigin, nestOtpe) =>
          try nested.monadMap(t => apply(t), otpe).transformed
          catch
            case ex: Throwable =>
              throw ex
    ...
    override def typeApply(orig: Term, targs: List[qctx.reflect.TypeTree], ntpe: TypeRepr): CpsTree =
      selects match
        case head :: tail =>
          if (head.targs.isEmpty) then copy(optOrigin = Some(orig), selects = head.copy(targs = targs) :: tail, otpe = ntpe)
          else throw MacroError("multiple type arguments are not supported", posExprs(orig))
        case Nil =>
    ...
    override def syncOrigin: Option[Term] =
      if (!isChanged && optOrigin.isDefined) then optOrigin
      else nested.syncOrigin.map(t => apply(t))
        ...
        case None =>
          nested match
            case MappedCpsTree(prevNested, prevF, prevNtpe) =>
              MappedCpsTree(prevNested, x => f(apply(prevF(x))), ntpe)
            case _ =>
              // MappedCpsTree(this, f, ntpe)
    ...
    override def monadFlatMap(f: Term => Term, ntpe: TypeRepr): CpsTree =
      nested.syncOrigin match
        ...
        case None =>
          nested match
            case MappedCpsTree(prevNested, prevF, prevNtpe) =>
              FlatMappedCpsTree(prevNested, x => f(apply(prevF(x))), ntpe)
            case _ =>
              FlatMappedCpsTree(this, f, ntpe)
  ...
  extension (otherCake: TreeTransformScope[?, ?, ?])
    ...
    def adoptStatement(t: qctx.reflect.Statement): otherCake.qctx.reflect.Statement =
      t.asInstanceOf[otherCake.qctx.reflect.Statement]
