...
trait InlinedTreeTransform[F[_], CT, CC <: CpsMonadContext[F]]:
  ...
  case class InlinedBindingsRecord(
      changes: HashMap[Symbol, InlinedBindingRecord],
      newBindings: List[Definition],
      awaitVals: List[ValDef]
  )

  def runInlined(origin: Inlined)(owner: Symbol): CpsTree =
    if (cpsCtx.flags.debugLevel >= 15) then cpsCtx.log(s"Inlined, origin=${safeShow(origin)}")
                ...
                case Lambda(params, body) =>
                  ...
                  if (cpsBinding.isAsync) then
                    ...
                    val newValDef = ValDef(
                      newSym,
                      cpsBinding.syncOrigin.map(body =>
                        Lambda(
                          newSym,
                          mt,
                ...
                case _ =>
                  val cpsRhs =
                    try {
                      ...
                      case ex: MacroError =>
                        report.warning(
                          s"error during transformation of valdef in inline, tpt=${tpt.show}\n, rhs=${rhs.show}\n, ex=${ex}",
                          posExprs(rhs)
                        )
                        throw ex
                    }
                  cpsRhs.syncOrigin match
                    case None =>
                      ...
                      if (cpsRhs.isLambda) {
                        ...
                        s.copy(newBindings = newValDef :: s.newBindings, changes = s.changes.updated(vx.symbol, bindingRecord))
                      } else {
                        ...
                        val awaitValSym =
                          Symbol.newVal(Symbol.spliceOwner, name + "$await", tpt.tpe, Flags.EmptyFlags, Symbol.noSymbol)
                        val awaitVal = ValDef(
                          awaitValSym,
                          Some(
        ...
        val transformer = new TreeMap() {
          ...
          override def transformTerm(term: Term)(owner: Symbol): Term =
            try transformTermInternal(term)(owner)
              ...
              case ex: MacroError =>
                if (cpsCtx.flags.debugLevel > 0) then report.warning(s"failed term: ${term}")
                throw ex
                  ...
                  case Some(binding) =>
                    binding match
                      case funBinding: InlinedFunBindingRecord =>
                        ...
                        if (funBinding.cpsTree.isAsync) then generateAwaitFor(changed, changed.tpe)
                        else changed
                      case _ =>
              ...
              case Apply(Select(obj @ Ident(name), "apply"), args) =>
                funValDefs.changes.get(obj.symbol) match
                  case Some(binding: InlinedFunBindingRecord) =>
                    ...
                    if (binding.cpsTree.isAsync) then generateAwaitFor(changed, changed.tpe)
                    else changed
                  case _ =>
                    super.transformTerm(term)(owner)
              case obj @ Ident(name) =>
                          ...
                          checkLambdaDef(rhs) match
                            case Some(Lambda(params, body)) =>
                              ...
                              Lambda(
                                ...
                                mt,
                                (owner, args) =>
                                  (if inlineProxies then TransformUtil.substituteLambdaParams(params, args, body, owner)
                                   else Apply(Select.unique(Ref(binding.newSym), "apply"), args.map(_.asInstanceOf[Term])))
                                    .changeOwner(owner)
                              )
                            case _ =>
                              throw MacroError("Lambda in rhs expected", posExprs(rhs))
                        else Ref(binding.newSym)
    ...
    if (cpsCtx.flags.debugLevel >= 15) then cpsCtx.log(s"runInline, cpsBody=${cpsBody}")
    if (origin.bindings.isEmpty) then cpsBody
    else InlinedCpsTree(owner, origin, funValDefs.newBindings.reverse, cpsBody)
...
object InlinedTreeTransform:
  ...
  def run[F[_]: Type, T: Type, C <: CpsMonadContext[F]: Type](using
      qctx1: Quotes
  )(cpsCtx1: TransformationContext[F, T, C], inlinedTerm: qctx1.reflect.Inlined): CpsExpr[F, T] = {
