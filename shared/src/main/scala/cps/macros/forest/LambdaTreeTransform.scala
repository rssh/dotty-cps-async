...
trait LambdaTreeTransform[F[_], CT, CC <: CpsMonadContext[F]]:
  ...
  def runLambda(lambdaTerm: Term, params: List[ValDef], body: Term)(owner: Symbol): CpsTree =
    ...
    val retval = if (cpsBody.isAsync) {
      ...
      if (cpsCtx.flags.allowShiftedLambda) then AsyncLambdaCpsTree(owner, lambdaTerm, params, cpsBody, lambdaTerm.tpe)
      else throw MacroError("await inside lambda functions without enclosing async block", lambdaTerm.asExpr)
      ...
      val newLambda = Lambda(
        owner,
        mt,
        (owner, args) => TransformUtil.substituteLambdaParams(params, args, changedBody, owner).changeOwner(owner)
      )
