...
trait RootTreeTransform[F[_], CT, CC <: CpsMonadContext[F]]:
        ...
        term match
          case lambdaTerm @ Lambda(params, body) =>
            ...
            B2.inNestedContext(
              ...
              scope =>
                scope
                  .runLambda(
                    lambdaTerm.asInstanceOf[scope.qctx.reflect.Term],
          ...
          case applyTerm @ Apply(fun, args) =>
            ...
            val tree = B2.inNestedContext(
              ...
              scope =>
                scope
                  .runApply(
                    applyTerm.asInstanceOf[scope.qctx.reflect.Apply],
          ...
          case inlined @ Inlined(call, bindings, body) =>
            val tree = B2.inNestedContext(
              ...
              scope =>
                scope
                  .runInlined(inlined.asInstanceOf[scope.qctx.reflect.Inlined])(owner.asInstanceOf[scope.qctx.reflect.Symbol])
                  .inCake(thisTransform)
            )
            tree
          ...
          case tpApply @ TypeApply(qual, tps) =>
            val tree = B2.inNestedContext(
              ...
              scope =>
                scope
                  .runTypeApply(
                    tpApply.asInstanceOf[scope.qctx.reflect.TypeApply],
          ...
          case _ => // TODO: elimi
            ...
            TransformUtil.veryWiden(term.tpe).asType match
              case '[et] =>
                val termExpr =
                  try term.asExprOf[et]
                  catch
                    case NonFatal(ex) =>
                      println(s"Can't cast term ${TransformUtil.safeShow(term)} to ${Type.show[et]}")
                      println(s"raw term=${term}")
                      println(
                        s"term.tpe=${term.tpe.show}, term.tpe.widen=${term.tpe.widen.show}, term.tpe.veryWiden=${TransformUtil.veryWiden(term.tpe).show}"
                      )
                ...
                val rCpsExpr =
                  try {
                    if cpsCtx.flags.debugLevel >= 15 then cpsCtx.log(s"nextedTransfornm: orin = $term")
  ...
  def runRootUneta(term: qctx.reflect.Term, muted: Boolean)(owner: Symbol): CpsTree = {
    ...
    val r = term match {
      ...
      case applyTerm @ Apply(x, args) =>
        ...
        nestScope
          .runApply(
            applyTerm.asInstanceOf[nestScope.qctx.reflect.Apply],
            x.asInstanceOf[nestScope.qctx.reflect.Term],
            args.asInstanceOf[List[nestScope.qctx.reflect.Term]],
            Nil
          )(owner.asInstanceOf[nestScope.qctx.reflect.Symbol])
          .inCake(thisTransform)
      case typeApply @ TypeApply(tfun, targs) =>
        ...
        nestScope
          .runTypeApply(
            typeApply.asInstanceOf[nestScope.qctx.reflect.TypeApply],
  ...
  object B2 {
    ...
    def inNestedContext(term: Term, owner: Symbol, muted: Boolean, op: TreeTransformScope[F, ?, ?] => CpsTree): CpsTree =
