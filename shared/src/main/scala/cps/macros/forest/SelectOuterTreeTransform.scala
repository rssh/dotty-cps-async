...
trait SelectOuterTreeTransform[F[_], CT, CC <: CpsMonadContext[F]]:
  ...
  def runSelectOuter(term: SelectOuter)(owner: Symbol): CpsTree =
    ...
    if (!qual.isChanged)
      ...
      SelectTypeApplyCpsTree.create(
        Some(term),
        qual,
        List(),
        List(SelectTypeApplyRecord(term.qualifier.tpe, term.symbol, List(), term.level)),
        term.tpe
      )
...
object SelectOuterTreeTransform:
  ...
  def run[F[_]: Type, T: Type, C <: CpsMonadContext[F]: Type](using
      qctx1: Quotes
  )(cpsCtx1: TransformationContext[F, T, C], selectOuterTerm: qctx1.reflect.SelectOuter): CpsExpr[F, T] = {
