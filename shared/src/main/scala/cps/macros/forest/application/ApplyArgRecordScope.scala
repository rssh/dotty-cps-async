...
trait ApplyArgRecordScope[F[_], CT, CC <: CpsMonadContext[F]]:
  ...
  case class ApplyArgsSummaryPropertiesStep1(
    ...
    def merge(other: ApplyArgRecord): ApplyArgsSummaryPropertiesStep1 =
      ApplyArgsSummaryPropertiesStep1(
        ...
        if (cpsDirectArg.isEmpty) then if (other.isCpsDirect) then Some(other.term) else None
        else cpsDirectArg
      )
  ...
  case class ApplyArgRepeatRecord(
    ...
    private def shiftedElemTpt: TypeTree =
      ...
      else shiftedLambdaTypeTree(repeatedTerm.elemtpt)
    ...
    private def shiftSeqTypeTree: TypeTree =
      ...
      seqTypeTree match
        case Applied(rep, args) =>
          if (rep.tpe.typeSymbol == defn.RepeatedParamClass) then Inferred(rep.tpe.appliedTo(shiftedElemTpt.tpe))
          else notFoundBackup
        case Inferred() =>
          seqTypeTree.tpe match
            case AppliedType(rep, List(v)) =>
              if (rep.typeSymbol == defn.RepeatedParamClass) then Inferred(rep.appliedTo(shiftedElemTpt.tpe))
              else notFoundBackup
        case _ =>
          notFoundBackup
  ...
  case class ApplyArgPrecalcTermRecord(
    ...
    def identArg(existsAsync: Boolean): Term =
      if (existsAsync) then ident
      else if termCpsTree.isChanged then termCpsTree.syncOrigin.get
      else term
  ...
  case class ApplyArgLambdaRecord(
    ...
    private def createAsyncPartialFunction(
        from: TypeRepr,
        to: TypeRepr,
        body: Match,
        params: List[ValDef],
      ...
      if (cpsCtx.flags.debugLevel >= 15) then cpsCtx.log(s"createAsyncPartialFunction: from = $from, to=$to")
      ...
      def newCheckBody(inputVal: Term): Term =
        ...
        @tailrec
        def transformCases(rest: List[CaseDef], acc: List[CaseDef], wasDefault: Boolean): List[CaseDef] =
          rest match
            case h :: t =>
            ...
            case Nil =>
              val lastCase = casePattern match
                ...
                case _ =>
                  throw MacroError(
                    "Internal error: case pattern should be Inlined(_,_,Match) pr Match, we have $casePattern",
                    posExpr(term)
                  )
              ;
                    ...
                    ${
                      ...
                      nBody match
                        case m @ Match(scr, caseDefs) =>
                          ...
                          val nCaseDefs = caseDefs.map(cd => rebindCaseDef(cd, cd.rhs, b0, true, Symbol.spliceOwner))
  ...
  case class ApplyArgByNameRecord(
      term: Term,
      index: Int,
      cpsTree: CpsTree,
      optShiftType: Option[ApplicationShiftType],
  ...
  def termIsNoOrderDepended(x: Term): Boolean =
    x match {
      case Literal(_) => true
      case Ident(_) =>
        if (x.symbol.isValDef) then x.symbol.flags.is(Flags.Mutable)
        else if x.symbol.isDefDef then true
        else if x.symbol.isBind then true
        else false
      case _ => false
    }
