...
object CollectionHelper {
  ...
  def retrieveIterableFactoryImpl[CC[x] <: Iterable[x]: Type](using Quotes): Expr[IterableFactory[CC]] = {
    ...
    if (companion.tpe <:< TypeRepr.of[IterableFactory[CC]]) then companion.asExprOf[IterableFactory[CC]]
    else report.errorAndAbort(s"companion for ${ccTpe.show} is not an IterableFactory")
  }
