...
class ArrayOpsAsyncShift[A] extends AsyncShift[ArrayOps[A]] {
  ...
  def map[F[_], B](arr: ArrayOps[A], monad: CpsMonad[F])(f: A => F[B])(using ct: ClassTag[B]): F[Array[B]] = {
    ...
    val fu = mappedArr.zipWithIndex.foldLeft(monad.pure(())) { case (s, (fa, i)) =>
      monad.flatMap(s)(_ => monad.map(fa)(b => { r(i) = b }))
    }
    monad.map(fu)(_ => r)
  }

  def flatMapIterableOnce[F[_], B](arr: ArrayOps[A], monad: CpsMonad[F])(
      f: A => F[IterableOnce[B]]
  )(using ct: ClassTag[B]): F[Array[B]] = {
  ...
  def flatMap[F[_], BS, B](arr: ArrayOps[A], monad: CpsMonad[F])(
      f: A => F[BS]
  )(using asIterableF: BS => F[Iterable[B]], ct: ClassTag[B]): F[Array[B]] = {
  ...
  def exists[F[_]](arr: ArrayOps[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[Boolean] = {
    def steps(it: Iterator[A]): F[Boolean] =
      if !it.hasNext then monad.pure(false)
      else
        val e = it.next
        monad.flatMap(p(e)) { v =>
  ...
  def find[F[_]](arrOps: ArrayOps[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[Option[A]] = {
    def steps(it: Iterator[A]): F[Option[A]] =
      if !it.hasNext then monad.pure(None)
      else
        val e = it.next
        monad.flatMap(p(e)) { v =>
  ...
  def filter[F[_]](arrOps: ArrayOps[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[Array[A]] = {
    ...
    def steps(it: Iterator[A], i: Int): F[Array[A]] =
      if !it.hasNext then monad.pure(tmp.slice(0, i))
      else
        val e = it.next
        monad.flatMap(p(e)) { v =>
          if v then
  ...
  def forall[F[_]](arrOps: ArrayOps[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[Boolean] = {
    def step(it: Iterator[A]): F[Boolean] =
      if !it.hasNext then monad.pure(true)
      else
  ...
  def groupBy[F[_], K](arrOps: ArrayOps[A], monad: CpsMonad[F])(f: A => F[K]): F[Map[K, Array[A]]] = {
    def step(it: Iterator[A], gathered: Map[K, ArrayBuffer[A]]): F[Map[K, ArrayBuffer[A]]] =
      if !it.hasNext then monad.pure(gathered)
      else
        val e = it.next
        monad.flatMap(f(e)) { k =>
  ...
  def groupMap[F[_], K, B](arrOps: ArrayOps[A], monad: CpsMonad[F])(
      key: A => F[K]
  )(f: A => F[B])(using ClassTag[B]): F[Map[K, Array[B]]] = {
  ...
  def indexWhere[F[_]](arrOps: ArrayOps[A], monad: CpsMonad[F])(p: A => F[Boolean], from: Int): F[Int] = {
    def steps(it: Iterator[A], i: Int): F[Int] =
      if !it.hasNext then monad.pure(-1)
      else
        val e = it.next
        monad.flatMap(p(e)) { v =>
  ...
  def partition[F[_]](arrOps: ArrayOps[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[(Array[A], Array[A])] = {
    def steps(it: Iterator[A], b1: ArrayBuffer[A], b2: ArrayBuffer[A]): F[(ArrayBuffer[A], ArrayBuffer[A])] = {
      if !it.hasNext then monad.pure((b1, b2))
      else
        val e = it.next
        monad.flatMap(p(e)) { v =>
  ...
  def partitionMap[F[_], A1, A2](arrOps: ArrayOps[A], monad: CpsMonad[F])(
      f: A => F[Either[A1, A2]]
  )(using ClassTag[A1], ClassTag[A2]): F[(Array[A1], Array[A2])] = {
...
class ArrayOpsWithFilterAsyncSubst[F[_], A](ops: ArrayOps[A], monad: CpsMonad[F], p: A => F[Boolean])
    extends CallChainAsyncShiftSubst[F, ArrayOps.WithFilter[A], F[ArrayOps.WithFilter[A]]] {
  ...
  def runBuild[B: ClassTag](it: Iterator[A], builder: ArrayBuilder[B])(step: (A, ArrayBuilder[B]) => F[Unit]): F[ArrayBuilder[B]] =
    if (it.hasNext) then
      ...
      monad.flatMap(p(e)) { v =>
        ...
        else runBuild(it, builder)(step)
      }
    else monad.pure(builder)
  ...
  def withFilter(q: A => Boolean): ArrayOpsWithFilterAsyncSubst[F, A] =
    new ArrayOpsWithFilterAsyncSubst(
      ops,
      monad,
      { a =>
        monad.map(p(a))(c => if (c) then q(a) else false)
      }
    )
  ...
  def withFilter_async(q: A => F[Boolean]): ArrayOpsWithFilterAsyncSubst[F, A] =
    new ArrayOpsWithFilterAsyncSubst(
      ...
      monad,
      { a =>
        monad.flatMap(p(a))(c => if (c) then q(a) else monad.pure(false))
      }
    )
