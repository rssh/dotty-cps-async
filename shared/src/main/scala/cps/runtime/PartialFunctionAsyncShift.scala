...
trait PartialFunctionCallChainSubst[F[+_], A, B](m: CpsMonad[F])
    extends CallChainAsyncShiftSubst[F, PartialFunction[A, B], A => F[B]]:
  ...
  def lift: CallChainAsyncShiftSubst[F, A => Option[B], A => F[Option[B]]] =
    Function1AndThenCallChainSubst[F, A, A, Option[B]](
      identity,
  ...
  def andThen[C](g: PartialFunctionCallChainSubst[F, B, C]): PartialFunctionCallChainSubst[F, A, C] =
    val r = new PartialFunctionCallChainSubst[F, A, C](m) {
      ...
      def isDefinedAt(x: A): F[Boolean] =
        m.flatMap(fThis.isDefinedAt(x))(fd => if (fd) then m.flatMap(fThis.apply(x))(g.isDefinedAt) else m.pure(false))

      def applyContOrElse[A1 <: A, D](x: A1, ifApply: C => D, ifNot: A1 => D): F[D] =
        fThis.applyContOrElse_async(x, y => g.applyContOrElse(y, ifApply, _ => ifNot(x)), x1 => m.pure(ifNot(x1)))

      def applyContOrElse_async[A1 <: A, D](x: A1, ifApply: C => F[D], ifNot: A1 => F[D]): F[D] =
        fThis.applyContOrElse_async(x, y => g.applyContOrElse_async(y, ifApply, _ => ifNot(x)), ifNot)
  ...
  def orElse[A1 <: A, B1 >: B](g: PartialFunctionCallChainSubst[F, A1, B1]): PartialFunctionCallChainSubst[F, A1, B1] =
    new PartialFunctionCallChainSubst[F, A1, B1](m) {
      ...
      def applyContOrElse[A2 <: A1, C](x: A2, ifApply: B1 => C, ifNot: A2 => C): F[C] =
        fThis.applyContOrElse_async[A2, C](x, b1 => m.pure(ifApply(b1)), x2 => g.applyContOrElse[A2, C](x2, ifApply, ifNot))
      ...
      def applyContOrElse_async[A2 <: A1, C](x: A2, ifApply: B1 => F[C], ifNot: A2 => F[C]): F[C] =
        fThis.applyContOrElse_async[A2, C](x, b1 => ifApply(b1), x2 => g.applyContOrElse_async[A2, C](x2, ifApply, ifNot))
