...
object UsingAsyncShift extends AsyncShift[Using.type]:
   ...
   def resources[F[_], R1, R2, R3, R4, A](o: Using.type, m:CpsTryMonad[F])(r1: R1, 
             r2: () => F[R2], r3: ()=>F[R3], r4: ()=>F[R4])
                (f: (R1, R2, R3, R4) => F[A])
                (implicit r1Rl: Using.Releasable[R1], r2Rl: Using.Releasable[R2], 
                          r3Rl: Using.Releasable[R3], r4Rl: Using.Releasable[R4]): F[A] =
